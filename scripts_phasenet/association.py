...
ncpu = 20
picks = pd.read_csv('PhaseNetPicks.csv')
...
config["max_sigma11"] = 10000
config["max_sigma22"] = 10000
config["max_sigma12"] = 10000
config["ncpu"] = ncpu
...
with open('catalogsPhase_gamma.csv', 'w') as fp:
    catalogs.to_csv(fp, sep="\t", index=False, 
                float_format="%.3f",
                date_format='%Y-%m-%dT%H:%M:%S.%f')
with open('picksPhase_gamma.csv', 'w') as fp:
...
TP, FN, FP = _CalFscore(det = list(catalogs['time']), obs = event['time'], start =  UTCDateTime(2019,1,1), end = UTCDateTime(2019, 2, 1), threshold = 60)
