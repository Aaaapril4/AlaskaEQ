...
def process_kernel(index, net, sta, starttime, endtime, total):
    ...
    try:
        ...
        # if xml file cannot be found, try to fetch from iris
        getinv = 0
        i = 0
        while getinv == 0:
            try:
                ...
                if i >= 9:
                    logger.info(
                        f"[{rank}]: {index}/{total} !Connection Error: {net}.{sta} {starttime}->{endtime}")
                    break
                time.sleep(0.1)
            else:
                getinv = 1
        ...
        try:
            ...
            logger.info(
                        f"[{rank}]: {index}/{total} !Error finding instrumental response: {net}.{sta} {starttime}->{endtime}")
            return
