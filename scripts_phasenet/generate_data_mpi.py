...
def ProcessData(process_index_this_rank, processList, event, picks, station):
    ...
    for index in process_index_this_rank:
        ...
        if len(stainfo) == 0:
            logger.info(
                f"!Station out of region: {evid} {sta} {starttime}->{endtime}")
            continue
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        picks = picks[picks.evid.isin(event.evid)]
        picks['timestamp'] = picks['timestamp'].apply(lambda x: UTCDateTime(x))
        stead = h5py.File('/mnt/scratch/jieyaqi/alaska/final/Alaska.hdf5', 'w')
    ...
    if rank == 0:
        catalogs.to_csv('/mnt/scratch/jieyaqi/alaska/final/catalogs.csv', sep=",", index=False, 
                    date_format='%Y-%m-%dT%H:%M:%S.%f')
        phases.to_csv('/mnt/scratch/jieyaqi/alaska/final/phases.csv', sep=",", index=False, 
                    date_format='%Y-%m-%dT%H:%M:%S.%f')
