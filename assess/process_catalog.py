...
def calculate_accumulated(catalog: pd.DataFrame, filter_col: str, threshold: float, start: float, end: float) -> pd.DataFrame:
    ...
    catalog = catalog[catalog['latitude'] > 53.8]
...
if __name__ == '__main__':
    ...
    cat['time'] = cat['time'].swifter.apply(lambda x: UTCDateTime(x))
    cmt['time'] = cmt['time'].swifter.apply(lambda x: UTCDateTime(x))
    ...
    picks['timestamp'] = picks['timestamp'].swifter.apply(lambda x: UTCDateTime(x))
    ...
    isc_picks = isc_picks[isc_picks['event_index'].isin(cmt['isc_id'])]
    isc_picks['timestamp'] = isc_picks['timestamp'].swifter.apply(lambda x: UTCDateTime(x))
    ...
    cat = cat.merge(cmt, on='isc_id', how = 'left', suffixes = ('', '_isc'))
    cat = cat.drop(columns=['isc_id', 'longitude_isc', 'latitude_isc', 'depth', 'time_isc', 'time2simeonof_isc', 'time2sandpoint_isc', 'time2chignik_isc', 'time2background_isc', 'slab_normal_depth'])
    cat.rename({'event_index_isc': 'name_cmt'})
    ...
    event_num = calculate_accumulated(cat, 'time2sandpoint', 5, -100, 200)
    event_num.to_csv(workdir / 'sandpoint_100', index=False)
    ...
    event_num = calculate_accumulated(cat, 'time2sandpoint', 7, -100, 200)
    event_num.to_csv(workdir / 'sandpoint_100_7', index=False)
    event_num = calculate_accumulated(cat, 'time2chignik', 7, -20, 60)
    event_num.to_csv(workdir / 'chignik_20_7', index=False)
    event_num = calculate_accumulated(cat, 'time2simeonof', 7, -20, 60)
    event_num.to_csv(workdir / 'simeonof_20_7', index=False)

    event_num = calculate_accumulated(cat, 'time2sandpoint', 10, -100, 200)
    event_num.to_csv(workdir / 'sandpoint_100_10', index=False)
    event_num = calculate_accumulated(cat, 'time2chignik', 10, -20, 60)
    event_num.to_csv(workdir / 'chignik_20_10', index=False)
    event_num = calculate_accumulated(cat, 'time2simeonof', 10, -20, 60)
    event_num.to_csv(workdir / 'simeonof_20_10', index=False)
