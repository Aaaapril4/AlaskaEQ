...
gmt pscoast -R$R -J$J -W0.5p,darkgray -Swhite -A$coast -Y5i -Df -K -P > $PS
grep NA\/PA $bdlst2 | awk '{print $3,$4}' | gmt psxy -J -R -W1.5 -O -K >> $PS

awk -F, 'NR>1 {print $2, $3, $15}' $seisf | gmt psxy -R$R -J$J -Sc1p -Gdarkgray -K -O >> $PS
...
# Simeonof
awk -F, '$7>=0 && $7<='$range' && NR>1 && $11 {print $2, $3}' $seisf | gmt psxy -R$R -J$J -Sc3p -W0.5p,"#E02514" -K -O >> $PS
...
# # Sandpoint
awk -F, '$8>=0 && $8<='$range' && NR>1 && $11 {print $2, $3}' $seisf | gmt psxy -R$R -J$J -Sc3p -W0.5p,"#4E00F5" -K -O >> $PS
...
# Chiknik
awk -F, '$9>=0 && $9<='$range' && NR>1 && $11{print $2, $3}' $seisf | gmt psxy -R$R -J$J -Sc3p  -W0.5p,"#40A362" -K -O >> $PS
...
# sandpoint2023
awk -F, '$10>=0 && $10<='$range' && NR>1 && $11 {print $2, $3} ' $seisf | gmt psxy -R$R -J$J -Sc3p  -W0.5p,"#DE7017" -K -O >> $PS
echo -160.90 54.44 32 4.30 -3.66 -0.64 5.11 2.75 -1.56 26 X Y | gmt psmeca -J$J -R$R -Sm5p -Gorange -K -O >> $PS

# control
awk -F, 'NR>1 && $14 {print $2, $3} ' $seisf | gmt psxy -R$R -J$J -Sc3p  -W0.5p,black -K -O >> $PS
